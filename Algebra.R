...
sourceCpp("escalar_vector_sparse.cpp")
...
sourceCpp("vector_vector.cpp")
vector_vector(x = x, y = y) # saída é densa


# C++ (dois vetores esparsos)
sourceCpp("vector_vector_sparse.cpp")
...
# C++ (denso)
sourceCpp("prod_interno.cpp")
produto_interno(x = x, y = y)

# C++ (esparso)
sourceCpp("prod_interno_sparse.cpp")
...
# C++  (denso)
sourceCpp("vector_size.cpp")
vector_size(x)

# C++ (esparso)
sourceCpp("vector_size_sparse.cpp")
...
block_diag(A = A, p = 2, q = 3) # repita a matriz A duas vezes na linha e 3 na coluna
